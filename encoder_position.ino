/* Control the travelling distance and direction of Minibot
 *  with linear velocity calculated from encoder data and 
 *  angular velocity determined by experiment.
 *  
 *  Modified based on the PID code in motor.ino 
 */
...
double timeSec = 1.0; // time step for calculating speed in PID (not the real time step)
double timeMSec = 1000;

double distance = 0; // distanced travelled in a set direction
double linearSpeed0; // linear speed calculated from motor 0
double linearSpeed1; // linear speed calculated from motor 1

double k = 0.0019; // linear velocity = k * angular velocity, k is determined by experiment
...
double currentAngle = 0; // the absolute angle that Minibot is facing in degrees (with the initial direction as 0)
double omega = 160; // angular velocity of turning determined by experiment
...
/* Calculate the distance that Minibot has travelled in a set direction*/
...
/* Turn the Minibot slowly to the setAngle based on calculated time.*/
void turnSlowly(double setAngle){
    timeElapsed = 0;
    double t; // time for turning
    if (fmod(currentAngle+360-setAngle,360) < 180){
      //turn left
      t = fmod(currentAngle+360-setAngle,360)/omega*1000;
      while (timeElapsed < t){
       digitalWrite(motor0pin2, HIGH);
       analogWrite(motor0pin1, 100);
       analogWrite(motor1pin2, 100);
       digitalWrite(motor1pin1, HIGH);
      }
    } else {
      t = (fmod(currentAngle+360-setAngle,360)-180)/omega*1000;
      while (timeElapsed < t){
      //turn right
      analogWrite(motor0pin2, 100);
      digitalWrite(motor0pin1, HIGH);
...
/* Move the Minibot in direction of an absolute angle 
 * for setDistance in meters.*/
void move(double setDistance, double angle){
 ...
 void loop(){
  ...
  Serial.println(currentAngle);
  delay(5000);
  currentAngle = 0; // Reset currentAngle for multiple trials
}
